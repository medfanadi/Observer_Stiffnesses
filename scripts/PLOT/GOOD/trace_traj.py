...
plt.grid(True)
plt.legend(["Reference path"," Path with NL observer  "," Path without observer"],loc='upper left',fontsize=36)
plt.show()
...
plt.grid(True)
plt.legend(["$e_y$ with NL observer ","$e_y$ with averge estimation of $C_f$ and $C_r$"],loc='upper left',fontsize=35)
...
plt.grid(True)
plt.legend(["$e_\\psi$ with NL observer ","$e_\\psi$ with averge estimation of $C_f$ and $C_r$"],loc='upper left',fontsize=35)
plt.yticks( color='k', size=40)
plt.xticks( color='k', size=40)
...
yf4= butter_lowpass_filter(Obs[:,8], 0.02, 200, 6)
yf5= butter_lowpass_filter(Obs[:,9], 0.02, 200, 6)

yf2= butter_lowpass_filter(WithOb[:,8], 0.02, 200, 6)
yf3= butter_lowpass_filter(WithOb[:,9], 0.02, 200, 6)


plt.text(10,12.5,'$(b)$', fontsize=45)
 
plt.plot(s4[:],kk*yf4, 'b', linewidth=3.5)

plt.plot(s4[:],kk*yf5, 'b--', linewidth=3.5)

plt.plot(s2[:],kk*yf2, 'r', linewidth=3.5)



plt.plot(s2[:],kk*yf3, 'r--', linewidth=3.5)

plt.xlabel('Curvilinear abscissa  [m]', fontsize=40)
plt.ylabel('Steering angles [deg]', fontsize=40)
plt.grid(True)
plt.legend(["$\\delta_f$ with NL observer  ","$\\delta_r$ with NL observer  ","$\\delta_f$ without observer  ","$\\delta_r$ without observer"],fontsize=42)

plt.xlim((5, 118))   # set the xlim to xmin, xmax
...
plt.ylabel('Slip angles [$deg$]', fontsize=45)
plt.grid(True)
plt.legend(["$\\beta_f$ with NL observer  ","$\\beta_r$ with NL observer ","$\\beta_f$ without observer  ","$\\beta_r$ without observer "],fontsize=45)
...
plt.grid(True)
plt.legend(["$F_{yF}$ with NL observer  ","$F_{yF}$ with averge estimation of $C_f$ and $C_r$  ","$F_{yR}$ with NL observer  ","$F_{yR}$ with averge estimation of $C_f$ and $C_r$  "],fontsize=28)
